...
public class CdnLoaderWorker : BackgroundService
{
    ...
    private const string CdnUrl = @"https://cdn.crpt.ru/api/v4/true-api/cdn/info";
    private const string HealthCheckAddress = @"api/v4/true-api/cdn/health/check";
    ...
    private async Task<List<CdnHost>> LoadCdNsAsync(string xapikey)
    {
        ...
        cdns = await HttpHelpers.GetJsonFromHttpAsync<CdnListAnswerTrueApi>(CdnUrl,
            Headers(xapikey),
            _httpClientFactory,
    ...
    private async Task<int> CheckHostHealthAsync(CdnHost cdn, string xapikey)
    {
        ...
        try
        {
            cdnHealth = await HttpHelpers.GetJsonFromHttpAsync<CdnHealth>($"{cdn.Host}/{HealthCheckAddress}",
                Headers(xapikey),
                _httpClientFactory,
                TimeSpan.FromSeconds(_configuration.HttpRequestTimeouts.CdnRequestTimeout));
        }
